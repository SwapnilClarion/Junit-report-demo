...
describe('Validate user is able to edit shipping address ', () => {
  let enteredAddressFields: any = [];
  let shippingAddressFields: any = [];
  let updatedData: any;
  ...
  it('Should redirect on checkout page and click edit link from other address and edit the address ', () => {
    browser.waitUntil((): any => {
    ...
    checkoutPage.clickOnUseOtherAddressLink();
    browser.waitUntil(() => {
      return checkoutPage.useOtherAddress.isDisplayed();
    }, 5000);
    // After editing shipping address it takes the few time to reflect changes thats why added static wait.
    browser.pause(3000);
    shippingAddressFields = checkoutPage.getAddressUseOtherAddDropdownShipping();
    logger.info('====================================================');
    logger.info(
      'Updated  shipping Address fields are : ' + shippingAddressFields,
    );
    assert.equal(
      checkoutPage.validateEditedShippingAddressFields(updatedData),
      true,
      addressTestData.shippingAddressValidateMessage,
    );
    logger.info('Updated shipping address successfully.');
